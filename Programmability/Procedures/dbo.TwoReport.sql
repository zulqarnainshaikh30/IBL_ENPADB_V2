...
CREATE PROCEDURE [dbo].[TwoReport]
--@Date date
...
select A.* into #ExceptionFinalStatusType
from ExceptionFinalStatusType A 
INNER JOIN
(select distinct CustomerAcID from Pro.Accountcal_hist with (nolock) where EffectiveToTimeKey > @LastQtrTimekey)B 
ON A.ACID = B.CustomerAcID 



select distinct UCIF_ID,A.CustomerID,CustomerName
into #DimCustomer
				from CustomerBasicDetail  A
				INNER JOIN 
				(select distinct CustomerID,max(EffectiveTotimekey)Timekey 
				from CustomerBasicDetail 
				where CustomerID in (select distinct CustomerId from #ExceptionFinalStatusType)
				group by CustomerID)B 
				ON A.CustomerId = B.CustomerId and A.EffectiveToTimeKey = B.Timekey
				where A.CustomerId in (select distinct CustomerId from #ExceptionFinalStatusType)
...
,REPLACE(xyz.CustomerName,',','') as [Customer Name]
,B.BranchCode as [Branch Code]
,REPLACE(BranchName,',','') as  [Branch Name]
,Z.ACID as [Account No.]
...
ON			A.CustomerEntityID=B.CustomerEntityID 
and A.EffectiveFromTimeKey <= @Timekey and A.EffectiveToTimeKey >= @Timekey
LEFT JOIN	#DimCustomer xyz 
on Z.CustomerID = xyz.CustomerId
LEFT JOIN	DIMSOURCEDB src
	on b.SourceAlt_Key =src.SourceAlt_Key	
LEFT JOIN DIMPRODUCT PD
	ON PD.EffectiveToTimeKey=49999
...
A.[Asset Classification]= (CASE WHEN B.SMA_Class = 'SUB' THEN 'SUB-STANDARD' 
WHEN B.SMA_Class = 'DB1' THEN 'DOUBTFUL I' WHEN B.SMA_Class = 'DB2' 
THEN 'DOUBTFUL II' WHEN B.SMA_Class = 'DB3' THEN 'DOUBTFUL III' else B.SMA_Class end)
from #TWOReport A 
...
TRUNCATE TABLE TWOReport_Final
INSERT INTO TWOReport_Final
select *  from #TWOReport 

select * from TWOReport_Final
