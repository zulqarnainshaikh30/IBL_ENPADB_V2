...
CREATE PROC [dbo].[AccountLevelInUp]  
...
IF(@MocStatus='InProgress')
  ...
  End
	...
	Select  @CustomerEntityID= CustomerEntityID from AdvacBasicDetail
	                          where  CustomerACID=@AccountID
...
ELSE IF @OperationFlag=21 AND @AuthMode ='Y'   
  ...
  IF @CrModApBy= (select UserLoginID from DimUserInfo where IsChecker2='N' AND EffectiveToTimeKey>=49999 and UserLoginID=@CrModApBy group by UserLoginID)  
    ...
    END  
     ...
     IF (@CrModApBy =(Select CreatedBy from AccountLevelMOC_Mod where  CreatedBy=@CrModApBy  AND  AccountEntityID =@AccountEntityID  
 ...
 ELSE
 BEGIN
        IF (@CrModApBy =(Select ApprovedByFirstLevel from AccountLevelMOC_Mod where  ApprovedByFirstLevel=@CrModApBy  AND  AccountEntityID =@AccountEntityID
						and EffectiveToTimeKey = 49999 
						and AuthorisationStatus = '1A'  
...
---------------Added for Rejection Pop Up Screen  29/06/2020   ----------  
 ...
 ELSE IF @OperationFlag=16  
  ...
  IF (@CrModApBy =(Select CreatedBy from AccountLevelMOC_Mod   
...
BEGIN  
  ...
  IF (@CrModApBy =(Select CreatedBy from AccountLevelMOC_Mod where AuthorisationStatus IN ('1A') and CreatedBy=@CrModApBy
                                                                  AND  AccountEntityID =@AccountEntityID
									                                and   EffectiveToTimeKey=49999 
...
BEGIN
             ...
             IF (@CrModApBy =(Select ApprovedBy from AccountLevelMOC_Mod 
	...
	---set parameters and UPDATE mod table in case maker checker enabled
			IF @AuthMode='Y'
				BEGIN
					...
					SELECT @ExEntityKey= MAX(EntityKey) FROM AccountLevelMOC_MOd 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							--AND AccountID=@AccountID
							AND  AccountEntityID =@AccountEntityID
							AND AuthorisationStatus IN('NP','MP','DP','RM','1A')	
					...
					UPDATE DBO.AccountLevelMOC_MOd
