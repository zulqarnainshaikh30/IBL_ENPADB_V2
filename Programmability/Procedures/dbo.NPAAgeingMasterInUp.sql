...
GO
CREATE PROC [dbo].[NPAAgeingMasterInUp]
						
						 @NPAAlt_Key	Int	= 0
						,@BusinessRule	Varchar(1000) = ''
						,@RefValue		Varchar(1000) = ''
						,@NPAAgeingMaster_ChangeFields Varchar(100)=NULL
...
------------Added for Rejection Screen  29/06/2020   ----------
		...
		DECLARE			@Uniq_EntryID			int	= 0
				...
				SET @ScreenName = 'NPAAgeingMaster'
 ...
 SET @EffectiveFromTimeKey  = @TimeKey
	...
	PRINT 1
		-----CHECK DUPLICATE
		IF EXISTS(				                
					SELECT  1 FROM DimNPAAgeingMaster WHERE NPAAlt_Key=@NPAAlt_Key AND BusinessRule=@BusinessRule AND ISNULL(AuthorisationStatus,'A')='A' and EffectiveFromTimeKey <=@TimeKey AND EffectiveToTimeKey >=@TimeKey
					UNION
					SELECT  1 FROM DimNPAAgeingMaster_Mod  WHERE (EffectiveFromTimeKey <=@TimeKey AND EffectiveToTimeKey >=@TimeKey)
															AND NPAAlt_Key=@NPAAlt_Key AND BusinessRule=@BusinessRule
...
----------------------------------------
	...
	IF @OperationFlag =1 AND @AuthMode ='Y' -- ADD
		...
		-- DELETE WITHOUT MAKER CHECKER
						...
						UPDATE DimNPAAgeingMaster SET
									ModifiedBy =@Modifiedby 
									,DateModified =@DateModified 
									,EffectiveToTimeKey =@EffectiveFromTimeKey-1
								WHERE (EffectiveFromTimeKey=EffectiveFromTimeKey AND EffectiveToTimeKey>=@TimeKey) AND NPAAlt_Key=@NPAAlt_Key
...
--------------------------------
				...
				IF EXISTS(SELECT 1 FROM DimNPAAgeingMaster WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@Timekey) AND NPAAlt_Key=@NPAAlt_Key)
				BEGIN
					UPDATE DimNPAAgeingMaster
	...
	---set parameters and UPDATE mod table in case maker checker enabled
			IF @AuthMode='Y'
				BEGIN
					...
					Print 'C'
					SELECT @ExEntityKey= MAX(EntityKey) FROM DimNPAAgeingMaster_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND NPAAlt_Key=@NPAAlt_Key
							AND AuthorisationStatus IN('NP','MP','DP','RM','1A')	
					...
					SELECT @ExEntityKey= MIN(EntityKey) FROM DimNPAAgeingMaster_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND NPAAlt_Key=@NPAAlt_Key
							AND AuthorisationStatus IN('NP','MP','DP','RM','1A')	
		...
		-------DELETE RECORD AUTHORISE
					...
					BEGIN	
						...
						IF EXISTS(SELECT 1 FROM DimNPAAgeingMaster WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
										AND NPAAlt_Key=@NPAAlt_Key)
						BEGIN
								UPDATE DimNPAAgeingMaster
									SET AuthorisationStatus ='A'
				...
				BEGIN
						...
						IF EXISTS(SELECT 1 FROM DimNPAAgeingMaster WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
							...
							BEGIN
								...
								IF EXISTS(SELECT 1 FROM DimNPAAgeingMaster WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
												AND EffectiveFromTimeKey=@TimeKey AND NPAAlt_Key=@NPAAlt_Key)
									BEGIN
											PRINT 'BBBB'
										UPDATE DimNPAAgeingMaster SET
												 NPAAlt_Key				    = @NPAAlt_Key
											 ...
											 WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
												AND EffectiveFromTimeKey=@EffectiveFromTimeKey AND NPAAlt_Key=@NPAAlt_Key
									END	
...
IF @ErrorHandle=0
	...
	SET @Parameter3 = (select STUFF((	SELECT Distinct ',' +ChangeFields
											from DimNPAAgeingMaster_Mod where  NPAAlt_Key=@NPAAlt_Key
					...
					IF @OperationFlag IN(16,17,18,20,21) 
						BEGIN 
						       Print 'Authorised'
								...
								@MenuID=@MenuID,
								@ReferenceID=@NPAAlt_Key ,-- ReferenceID ,
								@CreatedBy=NULL,
