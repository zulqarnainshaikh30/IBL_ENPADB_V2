...
CREATE PROCEDURE [dbo].[GetUploadStatus]
	...
	ELSE if exists(
		 ...
		 BEGIN
				...
				BEGIN
					...
					BEGIN
						Select 
						'Validating Header and Sheet Name' ErrorMsg,
						'In Progress' Flag,
		 ...
		 BEGIN 
			...
			BEGIN
			 ...
			 BEGIN
		        ...
		        --WHERE A.ROW=1
		         WHERE FileNames=@FileName AND ISNULL(ERRORDATA,'')<>'' 
		         ORDER BY SR_No 
		      END
		     ELSE 
		     BEGIN 
				...
				,Srnooferroneousrows,'Validation'TableName
		         ...
		         (SELECT *,ROW_NUMBER() OVER(PARTITION BY ColumnName,ErrorData,ErrorType,FileNames ORDER BY ColumnName,ErrorData,ErrorType,FileNames )AS ROW 
		         FROM  dbo.MasterUploadData    )a 
		         WHERE A.ROW=1 AND FileNames=@FileName 
		--ORDER BY ErrorData DESC 
		 END
	 END
	ELSE
