...
GO
CREATE PROC [dbo].[ProductCode_MasterSearchList]
--Declare
			 ...
			 IF OBJECT_ID('TempDB..#temp') IS NOT NULL
                 ...
                 (
                     SELECT 
							...
							------New add 180220222
							,A.SchemeType 
							,A.FacilityType--(CASE WHEN A.FacilityType = 'TL' THEN 'LN' ELSE A.FacilityType END) FacilityType
							,A.ConvFactor
							
							--select *
                     FROM DimProduct A
					 Inner Join DIMSOURCEDB B           ----------   Previously it was inner join 12/01/2024
...
--,Change varchar(max)
                     ...
                     SELECT A.ProductAlt_Key,
							...
							,ISNULL(A.DateApproved,A.DateModifie) as ModAppDate
			                 ,a.Changefields
							 ,A.SchemeType 
							,(CASE WHEN A.FacilityType = 'TL' THEN 'LN' ELSE A.FacilityType END) FacilityType
							,A.ConvFactor
                     FROM DimProduct_Mod A
					Left Join DIMSOURCEDB B                              ----------   Previously it was inner join 12/01/2024
					 ON A.SourceAlt_Key=B.SourceAlt_Key
					 AND B.EffectiveFromTimeKey <= @TimeKey AND B.EffectiveToTimeKey >= @TimeKey 
                 ...
                 FROM
                 (
                     SELECT ROW_NUMBER() OVER(ORDER BY ProductAlt_Key) AS RowNumber, 
			 ...
			 IF OBJECT_ID('TempDB..#temp16') IS NOT NULL
                 ...
                 (
                     SELECT A.ProductAlt_Key,
							...
							,ISNULL(A.DateApproved,A.DateModifie) as ModAppDate
							,a.Changefields
							,A.SchemeType as   SchemeTypeAlt_Key
							,A.FacilityType as FacilityTypeAlt_Key
							,A.SchemeType 
							,A.FacilityType--(CASE WHEN A.FacilityType = 'TL' THEN 'LN' ELSE A.FacilityType END) FacilityType
							,A.ConvFactor
                     FROM DimProduct_Mod A
...
END
   END TRY
	...
	SELECT *, 'ProductMaster' AS TableName FROM MetaScreenFieldDetail WHERE ScreenName='Product Master' and  MenuId=14551
