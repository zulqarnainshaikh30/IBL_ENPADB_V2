...
CREATE PROC [dbo].[DimSchemeMaster_InUp]
						
						@SchemeMappingAlt_Key               INT=0
						,@SrcSysSchemeCode			        Varchar(30)=''
					   ,@SrcSysSchemeName	                Varchar(200)= ''
					   ,@SourceAlt_Key                      INT =0
					   ,@SchemeAlt_Key                      INT=0
					   ,@SchemeName                         Varchar(200)=''
					   ,@SchemeGroup                        Varchar(200)=''
					   ,@SchemeSubGroup                     Varchar(200)=''
	...
	PRINT 'A'
			...
			DECLARE @AppAvail CHAR
				...
				IF(@AppAvail='N')                         
					...
					END
/*
	select * into DimSchemeMapping_Mod  from DimScheme	 where 1=2
		
	Alter table	DimSchemeMapping_Mod
	ADD SourceAlt_Key INT

	Alter table	DimSchemeMapping
	ADD SchemeMappingAlt_Key INT

	alter table DimSchemeMapping
	drop column Scheme_Key 
	alter table DimSchemeMapping
	add Scheme_Key smallint identity(1,1)

------EXEC sp_rename 'DimSchemeMapping_Mod.Source_Key', 'SourceAlt_Key', 'COLUMN';
*/	
	...
	PRINT 1
		-----CHECK DUPLICATE
		IF EXISTS(				                
					SELECT  1 FROM DimSchemeMapping WHERE SchemeMappingAlt_Key=@SchemeMappingAlt_Key
					                                  and SrcSysSchemeCode=@SrcSysSchemeCode
					...
					UNION
					SELECT  1 FROM DimSchemeMapping_Mod  WHERE (EffectiveFromTimeKey <=@TimeKey AND EffectiveToTimeKey >=@TimeKey)
															AND SchemeMappingAlt_Key=@SchemeMappingAlt_Key
															and SrcSysSchemeCode=@SrcSysSchemeCode
													        and SourceAlt_Key=@SourceAlt_Key
...
----------------------------------------
	...
	IF @OperationFlag =1 AND @AuthMode ='Y' -- ADD
		BEGIN
					 ...
					 SET @SchemeMappingAlt_Key = (Select ISNULL(Max(SchemeMappingAlt_Key),0)+1 from 
												(Select SchemeMappingAlt_Key from DimSchemeMapping
												 UNION 
												 Select SchemeMappingAlt_Key from DimSchemeMapping_Mod
												)A)
...
--------------------------------
				...
				IF EXISTS(SELECT 1 FROM DimSchemeMapping WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@Timekey) 
				                                              AND SchemeMappingAlt_Key=@SchemeMappingAlt_Key)
				BEGIN
					UPDATE DimSchemeMapping
	...
	---set parameters and UPDATE mod table in case maker checker enabled
			IF @AuthMode='Y'
				BEGIN
					...
					Print 'C'
					SELECT @ExEntityKey= MAX(Scheme_Key) FROM DimSchemeMapping_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND SchemeMappingAlt_Key=@SchemeMappingAlt_Key
							AND AuthorisationStatus IN('NP','MP','DP','RM')	
					...
					SELECT	@DelStatus=AuthorisationStatus,@CreatedBy=CreatedBy,@DateCreated=DATECreated
					 ...
					 FROM DimSchemeMapping_Mod
						WHERE SchemeMappingAlt_Key=@ExEntityKey
					...
					SELECT @ExEntityKey= MIN(Scheme_Key) FROM DimSchemeMapping_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND SchemeMappingAlt_Key=@SchemeMappingAlt_Key
							AND AuthorisationStatus IN('NP','MP','DP','RM')	
		...
		-------DELETE RECORD AUTHORISE
					...
					BEGIN	
						...
						SET AuthorisationStatus ='A'
							...
							,DateApproved=@DateApproved
							,EffectiveToTimeKey =@EffectiveFromTimeKey -1
						WHERE SchemeMappingAlt_Key=@SchemeMappingAlt_Key
							AND AuthorisationStatus in('NP','MP','DP','RM')
				...
				BEGIN
						...
						IF EXISTS(SELECT 1 FROM DimSchemeMapping WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
							...
							BEGIN
								...
								IF EXISTS(SELECT 1 FROM DimSchemeMapping WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
												AND EffectiveFromTimeKey=@TimeKey AND SchemeAlt_Key=@SrcSysSchemeCode)
									BEGIN
											PRINT 'BBBB'
										UPDATE DimSchemeMapping SET
												         SrcSysSchemeCode     =@SrcSysSchemeCode	
												        ,SourceAlt_Key             =@SourceAlt_Key      
												        ,SrcSysSchemeName     =@SrcSysSchemeName	
												        ,SchemeAlt_Key        =@SchemeAlt_Key    
												        ,SchemeName           =@SchemeName
														,SchemeMappingAlt_Key =@SchemeMappingAlt_Key  
														,SchemeGroup         =@SchemeGroup
														,SchemeSubGroup         =@SchemeSubGroup      
											 ...
											 WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
												AND EffectiveFromTimeKey=@EffectiveFromTimeKey AND SchemeMappingAlt_Key=@SchemeMappingAlt_Key
									END	
...
IF @ErrorHandle=0
	BEGIN
			INSERT INTO DimSchemeMapping_Mod  
											( 
												     SchemeAlt_Key
													,SchemeName
													,SourceAlt_Key
