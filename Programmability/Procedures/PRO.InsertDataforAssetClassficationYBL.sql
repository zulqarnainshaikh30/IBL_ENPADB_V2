...
EXEC [PRO].[InsertDataforAssetClassficationUTKS] @TIMEKEY=26343,@BRANCHCODE=NULL,@ISMOC='N'
=============================================*/
CREATE PROCEDURE [PRO].[InsertDataforAssetClassficationYBL] 
      @TIMEKEY INT 
...
 	 Asset_Norm='ALWYS_NPA'  
	,FinalAssetClassAlt_Key= CASE WHEN FinalAssetClassAlt_Key=1 THEN 
							 ...
							 END
	,FinalNpaDt= CASE WHEN FinalAssetClassAlt_Key=1 THEN ISNULL(StatusDate,@ProcessingDate) 
						WHEN FinalAssetClassAlt_Key>1 AND StatusDate IS NOT NULL and FinalNpaDt>=StatusDate then StatusDate
						WHEN FinalAssetClassAlt_Key>1 AND StatusDate IS NOT NULL  and FinalNpaDt<StatusDate then FinalNpaDt
					  ELSE @ProcessingDate 
				  END           
	,NPA_Reason='NPA DUE TO '+B.StatusType+' MARKING'
	,WriteOffAmount=(CASE WHEN b.Amount is NULL then ACL.Balance else  b.Amount END)
 FROM   PRO.AccountCal ACL INNER JOIN ExceptionFinalStatusType b
 on  ACL.AccountEntityID=b.AccountEntityID AND b.EFFECTIVEFROMTIMEKEY<=@TIMEKEY and b.EffectiveToTimeKey>=@TIMEKEY
 WHERE B.StatusType IN('TWO','WO','OTS','SALE TO ARC','FRAUD') 
...
 	 Asset_Norm='ALWYS_NPA'  
	,SysAssetClassAlt_Key= CASE WHEN SysAssetClassAlt_Key=1 THEN 
							 ...
							 END
	,SysNPA_Dt= CASE WHEN SysAssetClassAlt_Key=1 THEN ISNULL(StatusDate,@ProcessingDate)  
						WHEN SysAssetClassAlt_Key>1 AND StatusDate IS NOT NULL and SysNPA_Dt>=StatusDate then StatusDate
						WHEN SysAssetClassAlt_Key>1 AND StatusDate IS NOT NULL  and SysNPA_Dt<StatusDate then SysNPA_Dt
					  ELSE @ProcessingDate 
				  END           
...
/*
 ...
 FROM PRO.AccountCal A
 INNER JOIN UTKS_STGDB.DBO.[LMS_SECURITY_STG] B
 ON A.CustomerAcID=B.AccountID
 INNER  JOIN   UTKS_MISDB.[dbo].[DimCollateralSubType] C ON C.SrcSecurityCode=B.SecurityCode  
     AND C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey   
  WHERE C.Valid='Y'
 */
...
            WHERE RUNNINGPROCESSNAME='InsertDataforAssetClassficationUTKS'
