...
DISCRIPTION		:   Source to Stage: Control Data Report
*/

CREATE PROC [dbo].[Rpt-003]  
...
SELECT  
CONVERT(VARCHAR(15),Ext_Date,103)     AS Ext_Date,
SourceSystemName                      AS [Source System Name],
TA.TableName                          AS TableNames,
SRC_COUNT                             AS SourceCount,
STG_COUNT                             AS StageCount

FROM UTKS_STGDB..Table_audit	TA				

INNER JOIN UTKS_STGDB..ETLSolutionParameterforReport  ETLSPR   ON ETLSPR.TargetTableName=TA.TableName
                                                                    AND ETLSPR.EffectiveFromTimeKey<=@TimeKey	
														            AND ETLSPR.EffectiveToTimeKey>=@TimeKey

WHERE Ext_Date=@CurDate
