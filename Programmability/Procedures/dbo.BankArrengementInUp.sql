...
CREATE PROC [dbo].[BankArrengementInUp]
						...
						,@BankArrengement_changefieds varchar(100)=null
 ...
 SET @EffectiveFromTimeKey  = @TimeKey
	...
	PRINT 1
		...
		IF EXISTS(				                
					...
					SELECT  1 FROM DimBankingArrangement WHERE --BankingArrangementAlt_Key=@BankingArrangementAlt_Key
					...
					UNION
					SELECT  1 FROM DimBankingArrangement_Mod  WHERE (EffectiveFromTimeKey <=@TimeKey AND EffectiveToTimeKey >=@TimeKey)
															AND  ArrangementDescription=@ArrangementDescription
															AND   ISNULL(AuthorisationStatus,'A') in('NP','MP','DP','RM') 
					...
					IF @OperationFlag IN(16,17,18,20,21) 
						BEGIN 
						       Print 'Authorised'
								...
								@MenuID=@MenuID,
								@ReferenceID=@BankingArrangementAlt_Key ,-- ReferenceID ,
								@CreatedBy=NULL,
