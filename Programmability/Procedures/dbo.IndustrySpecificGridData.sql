...
CREATE PROCEDURE [dbo].[IndustrySpecificGridData]
	 @Timekey INT
...
BEGIN
   ...
   and UploadType =CASE WHEN @Menuid=24750 THEN 'Industry Specific Upload'
						ELSE  NULL END 
   )   A
...
BEGIN
  ...
  SELECT * INTO #INT FROM(
   ...
   FROM ExcelUploadHistory
   WHERE EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey>=@Timekey
   and UploadType =CASE WHEN @Menuid=1461 THEN 'Securitized Upload'
						ELSE  NULL END
