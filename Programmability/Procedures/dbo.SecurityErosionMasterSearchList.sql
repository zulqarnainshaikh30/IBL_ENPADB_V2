...
GO
CREATE PROC [dbo].[SecurityErosionMasterSearchList]
--Declare
													--@PageNo         INT         = 1, 
													--@PageSize       INT         = 10, 
													@OperationFlag  INT         = 1
...
/*  IT IS Used FOR GRID Search which are not Pending for Authorization And also used for Re-Edit    */
			...
			IF(@OperationFlag not in (16,17,20))
			 ...
			 IF OBJECT_ID('TempDB..#temp') IS NOT NULL
                 ...
                 FROM
                 (
                     SELECT ROW_NUMBER() OVER(ORDER BY SecurityAlt_Key) AS RowNumber, 
...
select *,'ErosionSecurityMaster' AS tableName from MetaScreenFieldDetail where ScreenName='Security Erosion Master'
