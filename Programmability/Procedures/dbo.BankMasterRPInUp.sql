...
CREATE PROC [dbo].[BankMasterRPInUp] 
						...
						,@BankMaster_changefieds varchar(100)=null
 ...
 SET @EffectiveFromTimeKey  = @TimeKey
	...
	PRINT 1
		...
		IF EXISTS(				                
					...
					SELECT  1 FROM DimBankRP WHERE BankCode=@BankShortName AND ISNULL(AuthorisationStatus,'A')='A' and EffectiveFromTimeKey <=@TimeKey AND EffectiveToTimeKey >=@TimeKey
					UNION
					SELECT  1 FROM DimBankRP_Mod  WHERE (EffectiveFromTimeKey <=@TimeKey AND EffectiveToTimeKey >=@TimeKey)
															AND BankCode=@BankShortName
															AND   ISNULL(AuthorisationStatus,'A') in('NP','MP','DP','RM') 
				)	
				BEGIN
				   PRINT 2
					...
					IF @OperationFlag IN(16,17,18,20,21) 
						BEGIN 
						       Print 'Authorised'
								...
								@MenuID=@MenuID,
								@ReferenceID=@BankRPAlt_Key ,-- ReferenceID ,
								@CreatedBy=NULL,
