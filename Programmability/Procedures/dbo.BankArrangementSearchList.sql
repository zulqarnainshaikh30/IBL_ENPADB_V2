...
GO
CREATE PROC [dbo].[BankArrangementSearchList] 
...
/*  IT IS Used FOR GRID Search which are not Pending for Authorization And also used for Re-Edit    */
			...
			IF(@OperationFlag not in (16,17,20))
			 ...
			 IF OBJECT_ID('TempDB..#temp') IS NOT NULL
                 ...
                 GROUP BY A.BankingArrangementAlt_Key,
							A.ArrangementDescription,
							A.AuthorisationStatus, 
                            A.EffectiveFromTimeKey, 
                            A.EffectiveToTimeKey, 
                 ...
                 FROM
                 (
                     SELECT ROW_NUMBER() OVER(ORDER BY BankingArrangementAlt_Key) AS RowNumber, 
   ...
   END;
	...
	END CATCH
	 ...
	 select *,'ArrangementMaster' AS tableName from MetaScreenFieldDetail where ScreenName='Banking Arrangement Master' 
     END;
GO