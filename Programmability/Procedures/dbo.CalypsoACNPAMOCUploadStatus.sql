...
--ACNPAMOCUploadStatus

-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

--exec [dbo].[CollateralDetailUploadStatus] @FileName =N'CollateralUpload (6).xlsx',@Level=N'VOD'
--go
...
CREATE PROCEDURE [dbo].[CalypsoACNPAMOCUploadStatus]
		 ...
		 BEGIN
			...
			BEGIN
			    PRINT 'Flag1'
			 	SELECT SR_No
				,ColumnName
				,(Case When ErrorData='No Record found' Then '' Else ErrorData END) as ErrorData
				,Case When ErrorType='No Record found' Then '' Else ErrorType END as ErrorType
				,FileNames
				,Flag
