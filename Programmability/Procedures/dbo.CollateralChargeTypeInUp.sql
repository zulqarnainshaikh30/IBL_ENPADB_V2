...
GO
CREATE PROC [dbo].[CollateralChargeTypeInUp]
						 @CollateralChargeTypeAltKey		 Int=0
...
------------Added for Rejection Screen  29/06/2020   ----------
		...
		DECLARE			@Uniq_EntryID			int	= 0
				...
				SET @ScreenName = 'CollateralChargeTypeMaster'
 ...
 SET @EffectiveFromTimeKey  = @TimeKey
	...
	PRINT 1
		-----CHECK DUPLICATE
		IF EXISTS(				                
					SELECT  1 FROM DimCollateralChargeType WHERE CollateralChargeTypeAltKey = @CollateralChargeTypeAltKey AND ISNULL(AuthorisationStatus,'A')='A' and EffectiveFromTimeKey <=@TimeKey AND EffectiveToTimeKey >=@TimeKey
					UNION
					SELECT  1 FROM DimCollateralChargeType_Mod  WHERE (EffectiveFromTimeKey <=@TimeKey AND EffectiveToTimeKey >=@TimeKey)
...
----------------------------------------
	...
	IF @OperationFlag =1 AND @AuthMode ='Y' -- ADD
		BEGIN
					 ...
					 SET @CollateralChargeTypeAltKey = (Select ISNULL(Max(CollateralChargeTypeAltKey),0)+1 from ( 
													Select CollateralChargeTypeAltKey from DimCollateralChargeType
													UNION 
													Select CollateralChargeTypeAltKey from DimCollateralChargeType_Mod)A)
		...
		-- DELETE WITHOUT MAKER CHECKER
						...
						UPDATE DimCollateralChargeType SET
									ModifiedBy =@Modifiedby 
									,DateModified =@DateModified 
									,EffectiveToTimeKey =@EffectiveFromTimeKey-1
								WHERE (EffectiveFromTimeKey=EffectiveFromTimeKey AND EffectiveToTimeKey>=@TimeKey) AND CollateralChargeTypeAltKey=@CollateralChargeTypeAltKey
...
--------------------------------
				...
				IF EXISTS(SELECT 1 FROM DimCollateralChargeType WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@Timekey) AND CollateralChargeTypeAltKey=@CollateralChargeTypeAltKey)
				BEGIN
					UPDATE DimCollateralChargeType
	...
	---set parameters and UPDATE mod table in case maker checker enabled
			IF @AuthMode='Y'
				BEGIN
					...
					Print 'C'
					SELECT @ExEntityKey= MAX(EntityKey) FROM DimCollateralChargeType_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND CollateralChargeTypeAltKey=@CollateralChargeTypeAltKey
							AND AuthorisationStatus IN('NP','MP','DP','RM')	
					...
					SELECT @ExEntityKey= MIN(EntityKey) FROM DimCollateralChargeType_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND CollateralChargeTypeAltKey=@CollateralChargeTypeAltKey
							AND AuthorisationStatus IN('NP','MP','DP','RM')	
		...
		-------DELETE RECORD AUTHORISE
					...
					BEGIN	
						...
						SET AuthorisationStatus ='A'
							...
							,DateApproved=@DateApproved
							,EffectiveToTimeKey =@EffectiveFromTimeKey -1
						WHERE CollateralChargeTypeAltKey=@CollateralChargeTypeAltKey
							AND AuthorisationStatus in('NP','MP','DP','RM')
						
						IF EXISTS(SELECT 1 FROM DimCollateralChargeType WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
										AND CollateralChargeTypeAltKey=@CollateralChargeTypeAltKey)
						BEGIN
								UPDATE DimCollateralChargeType
									SET AuthorisationStatus ='A'
				...
				BEGIN
						...
						IF EXISTS(SELECT 1 FROM DimCollateralChargeType WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
							...
							BEGIN
								...
								IF EXISTS(SELECT 1 FROM DimCollateralChargeType WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
												AND EffectiveFromTimeKey=@TimeKey AND CollateralChargeTypeAltKey=@CollateralChargeTypeAltKey)
									BEGIN
											PRINT 'BBBB'
										UPDATE DimCollateralChargeType SET
										         ChargeTypeID						= @ChargeTypeID
												,ChargeType							= @ChargeType
												,CollChargeDescription				= @CollChargeDescription
												,ModifiedBy							= @ModifiedBy
												,DateModified						= @DateModified
											 ...
											 WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
												AND EffectiveFromTimeKey=@EffectiveFromTimeKey AND CollateralChargeTypeAltKey=@CollateralChargeTypeAltKey
									END	
