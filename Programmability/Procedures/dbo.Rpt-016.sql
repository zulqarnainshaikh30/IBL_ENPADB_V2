...
DISCRIPTION		:   Base Prov Movement Format
*/

 create PROC [dbo].[Rpt-016]  
...
SELECT 
CONVERT(VARCHAR(20),ProvisionProcessDate,103)                       AS Created_Date,
CustomerAcid,
DACI.AssetClassGroup                                                AS I_AssetClassGroup,
CONVERT(VARCHAR(20),NPADt,103)                                      AS NPADt,
0                                                                   AS Opening_Gross_NPA,
...
ISNULL(ReductionDuetoUpgradeProvision,0)/@Cost                      AS ReductionDuetoUpgradeProvision,
ISNULL(ReductionDuetoRecovery_ExistingProvision,0)/@Cost            AS ReductionDuetoRecovery_ExistingProvision,
0                                                                   AS Write_Off,
ISNULL(ReductionProvisionDuetoWrite_Off,0)/@Cost                    AS ReductionProvisionDuetoWrite_Off,
0                                                                   AS SaletoARC,
ISNULL(ReductionProvisionDuetoRecovery_Arcs,0)/@Cost                AS ReductionProvisionDuetoRecovery_Arcs,
...
FROM ProvisionMovement PM 
LEFT JOIN AdvAcFinancialDetail ACFD        ON ACFD.RefSystemAcId=PM.CustomerACID
										       AND ACFD.EffectiveFromTimeKey<=@TimeKey 
										       AND ACFD.EffectiveToTimeKey>=@TimeKey



INNER JOIN DimAssetClass DACI              ON PM.InitialAssetClassAlt_Key=DACI.AssetClassAlt_Key
                                              AND DACI.EffectiveFromTimeKey<=@TimeKey 
											  AND DACI.EffectiveToTimeKey>=@TimeKey

INNER JOIN DimAssetClass DACF              ON PM.FinalAssetClassAlt_Key=DACF.AssetClassAlt_Key
