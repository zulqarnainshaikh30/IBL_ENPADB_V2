...
CREATE PROCEDURE [dbo].[ACLNPADATAOLDDATE]
AS
Declare @Date date = (select Date from Automate_Advances where Timekey = 26150)

Declare @Timekey int = (26150)
...
---------------------------------------------------ACL OUTPUT ---------------
	...
	delete from ACL_NPA_DATA where CONVERT(DATE,Process_Date,105) in (@Date)
...
select @Date as DateofData,'ACL_NPA_Data'TableName,count(1)Count from ACL_NPA_DATA where CONVERT(DATE,Process_Date,105) in (@Date)

	delete from ACL_UPG_DATA where CONVERT(DATE,Process_Date,105) in (@Date) 
