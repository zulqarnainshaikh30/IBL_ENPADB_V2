...
GO
CREATE PROCEDURE [dbo].[AccountFlaggingGetUploadStatus]
	 @FileName varchar(500)
	,@Level    varchar(500) 
AS
BEGIN 
	IF EXISTS(SELECT 1 FROM dbo.UploadStatus where FileNames=@FileName  AND @Level = 'Upload')
			BEGIN
	...
	ELSE if not exists( Select 1 from dbo.UploadStatus where FileNames=@FileName ) AND @Level='Upload'
		...
		BEGIN 
			...
			BEGIN
			  Select ErrorMsg,Flag,TableName,FileNames from dbo.ErrorMessageVOS where FileNames=@FileName  
			END
			ELSE
			BEGIN
		 ...
		 BEGIN
			...
			BEGIN
			 ...
			 --select * from MasterUploadData where ISNULL(ErrorData,'')<>''
		  ...
		  --     FROM dbo.MasterUploadData  
		         ;with CTE AS (
				 SELECT 
				 ROW_NUMBER() over (partition by SR_No order by SR_No) RN,
				 SR_No
				,ColumnName
				,ErrorData
				,ErrorType
			...
			select * from CTE ----updated by vinit
		...
		--WHERE A.ROW=1
		WHERE FileNames=@FileName AND RN=1
		AND ISNULL(ERRORDATA,'')<>''
