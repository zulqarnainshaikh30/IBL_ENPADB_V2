...
CREATE PROC [dbo].[GLProductMasterInUp] 
	  @GLProductAlt_Key							Int=0
	 ,@ProductCode								Varchar(30)=''
	 ,@ProductName								Varchar(200)=''
	 ,@SourceAlt_Key							Varchar(20)=''
	 ,@AssetGLCode_STD							Varchar(30)=''						
	 ,@AssetGLCode_NPA							Varchar(30)=''						
	 ,@InterestSuspenseNormal					Varchar(30)=''
	 ,@InterestReceivableNormal					Varchar(30)=''
	 ,@InterestIncomeNormal						Varchar(30)=''
	 ,@SuspendedInterestNormal					Varchar(30)=''
	 ,@InterestSuspensePenal					Varchar(30)=''
	 ,@InterestReceivablePenal					Varchar(30)=''
	 ,@InterestIncomePenal						Varchar(30)=''
	 ,@SuspendedInterestPenal					Varchar(30)=''
	 ,@Prov_Dr_GL								Varchar(30)=''
	 ,@Prov_Cr_GL  								Varchar(30)=''
	 ,@GLProductMaster_changeFields				varchar(100)=null
...
--------------------------------
	...
	BEGIN
		...
		UPDATE dbo.DimGLProduct_AU_Mod   
			...
			BEGIN
				...
				ELSE
					BEGIN
						--SET @ModifiedBy  =@CrModApBy
						--SET @DateModified =GETDATE()
						SELECT	@CreatedBy=CreatedBy, @DateCreated=DATECreated FROM DimGLProduct_AU 
						WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey >=@TimeKey )
							AND GLProductAlt_Key=@GLProductAlt_Key 
					   SET @ApprovedBy = @CrModApBy			
					   SET @DateApproved=GETDATE()
	...
	---set parameters and UPDATE mod table in case maker checker enabled
		...
		-------DELETE RECORD AUTHORISE
					...
					BEGIN
					 UPDATE DimGLProduct_AU_Mod
						 SET AuthorisationStatus ='A' --update by vinit
							,ModifiedBy =@CrModApBy  
							,ApprovedBy=@CrModApBy 
							,DateApproved = getdate()
					WHERE GLProductAlt_Key=@GLProductAlt_Key AND AuthorisationStatus in('NP','MP','RM','1A')
								...
								IF EXISTS(SELECT 1 FROM DimGLProduct_AU WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
									...
									print @GLProductAlt_Key
										...
										UPDATE DimGLProduct_AU SET
												...
												,AssetGLCode_NPA							=	@AssetGLCode_NPA
												,InterestSuspenseNormal						=	@InterestSuspenseNormal
												,InterestReceivableNormal					=	@InterestReceivableNormal
												,InterestIncomeNormal						=	@InterestIncomeNormal
												,SuspendedInterestNormal					=	@SuspendedInterestNormal
												,InterestSuspensePenal						=	@InterestSuspensePenal
												,InterestReceivablePenal					=	@InterestReceivablePenal
												,InterestIncomePenal						=	@InterestIncomePenal
												,SuspendedInterestPenal						=	@SuspendedInterestPenal
												,Prov_Dr_GL									=	@Prov_Dr_GL
												,Prov_Cr_GL									=	@Prov_Cr_GL
												,ModifiedBy									   = @ModifiedBy
												,DateModified								   = @DateModified
...
--END
	...
	SET @Parameter2 = (select STUFF((	SELECT Distinct ',' +ChangeFields
											from DimGLProduct_AU_Mod where GLProductAlt_Key=@GLProductAlt_Key
