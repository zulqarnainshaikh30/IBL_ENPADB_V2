...
GO
CREATE PROCEDURE [dbo].[AccountFlaggingGridData]
	 @Timekey INT
...
BEGIN
   ...
   CASE WHEN  AuthorisationStatus='A' THEN 'Authorized'
	...
	,ISNULL(ApprovedBy,ApprovedByFirstLevel) as CrAppBy
	,ISNULL(DateApproved,DateApprovedFirstLevel) as CrAppDate
   ...
   --                             END ASC
	--updated by vinit			
                                ...
                                FROM #INT1 Where AuthorisationStatus 
								Not In ('Authorized','Rejected','Pending')
  ...
  SELECT * INTO #INT FROM(
						...
						--,DateofUpload,
						   ...
						   and UploadType =CASE WHEN @Menuid=1470 THEN'Account Flagging Upload'
							...
							order by UniqueUploadID desc  --select * from ExcelUploadHistory order by UniqueUploadID desc
   ...
   --updated by vinit  
								...
								CrModBy,CrModDate,CrAppBy,CrAppDate,ModAppBy,ModAppDate
                                FROM #INT 
								Where AuthorisationStatus 
								Not In ('Authorized','Rejected','1Authorized')
                                order by
...
---------------comment by vinit---------------------------------
								--Where AuthorisationStatus
								--Not In ('Authorized','Rejected','1Authorized','Pending')
        --                         ORDER BY CASE WHEN AuthorisationStatus='Pending' THEN CAST(1 AS VARCHAR(50))
        --                        WHEN AuthorisationStatus='Authorized' THEN CAST(2 AS VARCHAR(50))
        --                        WHEN AuthorisationStatus='Rejected' THEN CAST(3 AS VARCHAR(50))
								--WHEN  AuthorisationStatus='1Authorized' THEN CAST(4 AS VARCHAR(50))
