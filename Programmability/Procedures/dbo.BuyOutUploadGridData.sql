...
CREATE PROCEDURE [dbo].[BuyOutUploadGridData]
	 @Timekey INT
...
print'1'
  ...
  SELECT * INTO #INT FROM(
   ...
   CASE WHEN  AuthorisationStatus='A' THEN 'Authorized'
	...
	--,IsNull(ModifyBy,CreatedBy)as CrModBy
	--,IsNull(DateModified,DateCreated)as CrModDate
	--,ISNULL(ApprovedBy,ModifyBy) as CrAppBy
	--,ISNULL(DateApproved,DateModified) as CrAppDate
	--,ISNULL(ApprovedBy,ApprovedByFirstLevel) as ModAppBy
	--,ISNULL(DateApproved,DateApprovedFirstLevel) as ModAppDate
	,IsNull(ModifyBy,CreatedBy)as CrModBy
	,IsNull(DateModified,DateCreated)as CrModDate
	,ISNULL(ApprovedBy,ApprovedByFirstLevel) as CrAppBy
	,ISNULL(DateApproved,DateApprovedFirstLevel) as CrAppDate
