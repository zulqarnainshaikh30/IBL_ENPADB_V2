...
Declare @TimeKey AS INT =(Select TimeKey from Automate_Advances where EXT_FLG='Y')
   ...
   IF (@SourceType ='LMS')
	...
	--------------Finacle
			...
			Select 'LMSUpgrade' AS TableName,A.DateofData AS [Date of Data],A.SourceSystemName [Source System] ,
  ...
  IF (@SourceType ='BRNET')
	BEGIN
	 --------------Ganaseva
	Select 'BRNETUpgrade' AS TableName, A.DateofData AS [Date of Data],A.SourceSystemName [Source System] ,
			A.CustomerID [Customer ID] , 
			A.AccountID [Account ID],'UTKS' [Bank ID],A.BranchCode SOL_ID,
			AB.SourceAssetClass as [Current Sub - Asset Class Code],
			A.AssetSubClass as [Revised Sub Asset Class Code],
	...
	BEGIN
	  --------------MiFin
Select 'PISMOUpgrade' AS TableName, A.DateofData AS [Date of Data],A.SourceSystemName [Source System] ,
			A.CustomerID [Customer ID] , 
			A.AccountID [Account ID],'UTKS' [Bank ID],A.BranchCode SOL_ID,
			AB.SourceAssetClass as [Current Sub - Asset Class Code],
			A.AssetSubClass as [Revised Sub Asset Class Code],
