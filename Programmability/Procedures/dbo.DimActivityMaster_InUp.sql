...
CREATE PROC [dbo].[DimActivityMaster_InUp]
						
						@SrcSysActivityCode			        Varchar(30)=''
					   ,@SourceAlt_Key                      INT =0
					   ,@SrcSysActivityName	                Varchar(200)	= ''
					   ,@ActivityAlt_Key                     INT=0
					   ,@ActivityName                        Varchar(200)=''
					   ,@ActivityMappingAlt_Key             INT=0
					   ,@DimActivityMaster_changeFields     Varchar(50)  =NULL
...
------------Added for Rejection Screen  29/06/2020   ----------
 ...
 SET @EffectiveFromTimeKey  = @TimeKey
	...
	--select * into DimActivityMapping  from DimActivity	 where 1=2
		
	--Alter table	DimActivityMapping
	--ADD Source_Key INT
...
--EXEC sp_rename 'DimActivityMapping_Mod.Source_Key', 'SourceAlt_Key', 'COLUMN';
	...
	PRINT 1
		-----CHECK DUPLICATE
		IF EXISTS(				                
					SELECT  1 FROM DimActivityMapping WHERE ActivityMappingAlt_Key=@ActivityMappingAlt_Key
					                                  and SrcSysActivityCode=@SrcSysActivityCode
					...
					UNION
					SELECT  1 FROM DimActivityMapping_Mod  WHERE (EffectiveFromTimeKey <=@TimeKey AND EffectiveToTimeKey >=@TimeKey)
															AND ActivityMappingAlt_Key=@ActivityMappingAlt_Key
															and SrcSysActivityCode=@SrcSysActivityCode
													        and SourceAlt_Key=@SourceAlt_Key
		...
		ELSE
			BEGIN
			    SET @ActivityMappingAlt_Key = (Select ISNULL(Max(ActivityMappingAlt_Key),0)+1 from 
												(Select ActivityMappingAlt_Key from DimActivityMapping
												 UNION 
												 Select ActivityMappingAlt_Key from DimActivityMapping_Mod
												)A)
			END
...
--------------------------------
				...
				IF EXISTS(SELECT 1 FROM DimActivityMapping WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@Timekey) 
				                                              AND ActivityMappingAlt_Key=@ActivityMappingAlt_Key)
				BEGIN
					UPDATE DimActivityMapping
...
ELSE IF @OperationFlag=21 AND @AuthMode ='Y' 
		...
		IF EXISTS(SELECT 1 FROM DimActivityMapping WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@Timekey) AND ActivityMappingAlt_Key=@ActivityMappingAlt_Key)
				BEGIN
					UPDATE DimActivityMapping
						SET AuthorisationStatus='A'
					WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
	...
	END
		...
		SET @DateApproved  = GETDATE()
						...
						SET AuthorisationStatus ='1A'
							,ApprovedBy=@ApprovedBy
							,DateApproved=@DateApproved
							WHERE ActivityMappingAlt_Key=@ActivityMappingAlt_Key
							AND AuthorisationStatus in('NP','MP','DP','RM')
	...
	---set parameters and UPDATE mod table in case maker checker enabled
			IF @AuthMode='Y'
				BEGIN
					...
					Print 'C'
					SELECT @ExEntityKey= MAX(Activity_Key) FROM DimActivityMapping_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND ActivityMappingAlt_Key=@ActivityMappingAlt_Key
							AND AuthorisationStatus IN('NP','MP','DP','RM','1A')	
					...
					SELECT	@DelStatus=AuthorisationStatus,@CreatedBy=CreatedBy,@DateCreated=DATECreated
					 ...
					 FROM DimActivityMapping_Mod
						WHERE Activity_Key=@ExEntityKey
					...
					SELECT @ExEntityKey= MIN(Activity_Key) FROM DimActivityMapping_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND ActivityMappingAlt_Key=@ActivityMappingAlt_Key
							AND AuthorisationStatus IN('NP','MP','DP','RM','1A')	
				...
				BEGIN
						...
						IF EXISTS(SELECT 1 FROM DimActivityMapping WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
							...
							BEGIN
								...
								IF EXISTS(SELECT 1 FROM DimActivityMapping WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
												AND EffectiveFromTimeKey=@TimeKey AND ActivityMappingAlt_Key=@ActivityMappingAlt_Key)
									BEGIN
											PRINT 'BBBB'
										UPDATE DimActivityMapping SET
												         SrcSysActivityCode     =@SrcSysActivityCode	
												        ,SourceAlt_Key             =@SourceAlt_Key      
												        ,SrcSysActivityName     =@SrcSysActivityName	
												        ,ActivityAlt_Key        =@ActivityAlt_Key    
												        ,ActivityName           =@ActivityName
														,ActivityMappingAlt_Key =@ActivityMappingAlt_Key       
											 ...
											 WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
												AND EffectiveFromTimeKey=@EffectiveFromTimeKey AND ActivityMappingAlt_Key=@ActivityMappingAlt_Key
									END	
								...
								IF @IsAvailable='N' OR @IsSCD2='Y'
									BEGIN
										INSERT INTO DimActivityMapping
												(
													 ActivityAlt_Key
													,ActivityName
													,SourceAlt_Key
...
IF @ErrorHandle=0
	BEGIN
			INSERT INTO DimActivityMapping_Mod  
											( 
												     ActivityAlt_Key
													,ActivityName
													,SourceAlt_Key
					...
					IF @OperationFlag IN(16,17,18,20,21) 
						BEGIN 
						       Print 'Authorised'
								...
								@MenuID=@MenuID,
								@ReferenceID=@SrcSysActivityCode ,-- ReferenceID ,
								@CreatedBy=NULL,
